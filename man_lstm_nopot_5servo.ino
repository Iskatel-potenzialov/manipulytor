...
// ============================================================================
// 2. КАНАЛЫ PCA9685 (5 сервоприводов)
// ============================================================================
...
#define SERVO_CHANNEL_4  0  // Серво 5 (захват)
...
float currentAngles[NUM_SERVOS] = {65, 115, 92, 110, 120}; // Home по умолчанию 

// ============================================================================
// 4. ОГРАНИЧЕНИЯ УГЛОВ (механические пределы серво)
// ============================================================================
const float SERVO_MIN_ANGLES[NUM_SERVOS] = {0, 40, 0, 0, 80};   // Мин углы (5-й: 50°)
const float SERVO_MAX_ANGLES[NUM_SERVOS] = {130, 170, 150, 170, 135}; // Макс углы (5-й: 90°)
...
// ============================================================================
// 8. ФУНКЦИЯ: Парсинг углов из команды SET_ANG (теперь 5 значений)
// ============================================================================
...
void safeShutdown() {
  ...
  // Безопасные углы (серво не перегружены) - теперь для 5 серво
  float safeAngles[NUM_SERVOS] = {0, 90, 45, 90, 90};
...
// ============================================================================
void goToHome() {
  float homeAngles[NUM_SERVOS] = {65, 115, 92, 110, 120}; // добавлен 5-й серво 90°
...
void setup() {
  ...
  Serial.println("🤖 Arduino манипулятор готов (5 серво, open-loop)");
  ...
  Serial.println("  SET_ANG,a1,a2,a3,a4,a5  - Установка углов (5 значений)");
...
void loop() {
  if (Serial.available() > 0) {
    ...
    // ========================================================================
    // Команда: SET_ANG,a1,a2,a3,a4,a5 (LSTM траектория)
    // ========================================================================
    if (command.startsWith("SET_ANG")) {
      ...
      if (parseAngles(command, angles)) {
        ...
        Serial.println("ERROR: Неверный формат. Пример: SET_ANG,65,115,92,110,90");
      }
    }

...
BLEND_CONFIG = {
    ...
    'start_step': 120,         # Начинать наведение за 80 шагов до конца 
}
...
POINT1_REAL_DEG = [107, 95, 87, 95, 90]     # Клетка h6, до захвата клешня 90°
...
# ============================================================================
# 6. ФУНКЦИИ ПРЕДСКАЗАНИЯ LSTM (без изменений, работают с 4 каналами)
# ============================================================================
...
# ----------------------------------------------------------------------------
# Функция выполнения одного перемещения (адаптирована для 5 серво)
# ----------------------------------------------------------------------------
def run_trajectory(start_real, target_real, description):
    """
    start_real, target_real — списки из 5 реальных углов (градусы).
    Для LSTM используются только первые 4.
    """
    ...
    trajectory = [start_real.copy()]  # для отладки
    ...
    final_real = trajectory_angles[-1] if MODE == 'REAL_ROBOT' else angles_real
...
# ----------------------------------------------------------------------------
if LSTM_AVAILABLE:
    # 1. HOME → POINT1 (клешня остаётся 110°)
    run_trajectory(HOME_REAL_DEG, POINT1_REAL_DEG, "HOME → POINT1")

    # После достижения POINT1: пауза 2 сек, затем захват (клешня 125°)
    if MODE == 'REAL_ROBOT':
        ...
        delay = robot.calculate_delay(robot.current_angles, new_angles)  # текущие углы уже обновлены после set_servo_angles? Нет, в set_servo_angles они обновляются. Но для расчёта задержки используем разницу между старыми и новыми.
        # Лучше вычислить задержку до отправки, используя предыдущие углы
        prev_angles = robot.current_angles.copy()
        robot.set_servo_angles(new_angles)
        delay = robot.calculate_delay(prev_angles, new_angles)
        time.sleep(delay)
        print("✅ Захват выполнен, угол клешни 125°")
    else:
    ...
    # 2. POINT1 → HOME (клешня остаётся 128°)
    ...
    # 3. HOME → POINT2 (клешня остаётся 128°, т.к. после захвата)

    if MODE == 'REAL_ROBOT':
        run_trajectory(robot.current_angles, POINT2_REAL_DEG, "HOME → POINT2")
    else:
       
        run_trajectory(HOME_REAL_DEG, POINT2_REAL_DEG, "HOME → POINT2")

    # После достижения POINT2: пауза 2 сек, затем отпускание (захват 90°)
    if MODE == 'REAL_ROBOT':
        ...
        delay = robot.calculate_delay(prev_angles, new_angles)
        time.sleep(delay)
        print("✅ Отпускание выполнено, угол захвата 90°")
    else:
    ...
    # 4. POINT2 → HOME (захват остаётся 90°)
    point2_after_release = POINT2_REAL_DEG.copy()
    point2_after_release[4] = 90.0
    if MODE == 'REAL_ROBOT':
        run_trajectory(robot.current_angles, HOME_REAL_DEG, "POINT2 → HOME")
    else:
        run_trajectory(point2_after_release, HOME_REAL_DEG, "POINT2 → HOME")
